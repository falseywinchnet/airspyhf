...
cc -O2 -Wall -Wextra -Wpedantic -fobjc-arc \
    -DAOB_FIFO_ONLY_VIEW \
    -I"$HERE" "$HERE/counter_window.m" \
    -framework Cocoa -o "$OUT/airspy-fifo"
...
mkdir -p "$OUT/AirspyFIFO.app/Contents/MacOS"
cp "$OUT/airspy-fifo" \
    "$OUT/AirspyFIFO.app/Contents/MacOS/airspy-fifo"
cp "$HERE/AirspyFIFO-Info.plist" \
    "$OUT/AirspyFIFO.app/Contents/Info.plist"
...
file "$OUT/airspy-helper" "$OUT/airspy-counters" "$OUT/airspy-fifo" \
